...
module IntSerialize : (SERIALIZE with type t = int) =  struct    type t = int    let serialize = string_of_int  end ;;
...
module IntStringSerialize =  struct    type t = (int * string)    let serialize (n, s) =      "(" ^ string_of_int n ^ ",'" ^ s ^ "')"  end ;;
